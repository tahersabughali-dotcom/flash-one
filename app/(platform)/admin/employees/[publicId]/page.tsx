...
import { getEmployee, listInternalNotes, listOperationalActivity } from "@/lib/server/operations";
...
export default async function Page({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  await requirePlatformAdmin(OPERATIONS_PATHS.employee(publicId));
  const record = await getEmployee(publicId);
  if (!record) notFound();
  const [notes, activity] = await Promise.all([
    listInternalNotes("employee", record.public_id),
    listOperationalActivity("employee", record.public_id),
  ]);
  return (
    <main>
      <PageHeader
      ...
      />
      <EmployeeForm record={record} />
      <SectionPanel title="Internal notes">
        <InternalNoteForm entityKind="employee" entityPublicId={record.public_id} />
        {notes.length === 0 ? (
