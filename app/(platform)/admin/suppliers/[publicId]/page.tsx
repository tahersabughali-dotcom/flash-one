...
import { getSupplier, listInternalNotes, listOperationalActivity } from "@/lib/server/operations";
...
export default async function Page({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  await requirePlatformAdmin(OPERATIONS_PATHS.supplier(publicId));
  const record = await getSupplier(publicId);
  if (!record) notFound();
  const [notes, activity] = await Promise.all([
    listInternalNotes("supplier", record.public_id),
    listOperationalActivity("supplier", record.public_id),
  ]);
  return (
    <main>
      <PageHeader
      ...
      />
      <SupplierForm record={record} />
      <SectionPanel title="Internal notes">
        <InternalNoteForm entityKind="supplier" entityPublicId={record.public_id} />
        {notes.length === 0 ? (
