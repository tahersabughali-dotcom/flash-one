...
import { getFreelancer, listInternalNotes, listOperationalActivity } from "@/lib/server/operations";
...
export default async function Page({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  await requirePlatformAdmin(OPERATIONS_PATHS.freelancer(publicId));
  const record = await getFreelancer(publicId);
  if (!record) notFound();
  const [notes, activity] = await Promise.all([
    listInternalNotes("freelancer", record.public_id),
    listOperationalActivity("freelancer", record.public_id),
  ]);
  return (
    <main>
      <PageHeader
      ...
      />
      <FreelancerForm record={record} />
      <SectionPanel title="Internal notes">
        <InternalNoteForm entityKind="freelancer" entityPublicId={record.public_id} />
        {notes.length === 0 ? (
