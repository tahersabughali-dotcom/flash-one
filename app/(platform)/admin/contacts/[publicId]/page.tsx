...
import { getContact, listInternalNotes, listOperationalActivity } from "@/lib/server/operations";
...
export default async function Page({ params }: { params: Promise<{ publicId: string }> }) {
  ...
  await requirePlatformAdmin(OPERATIONS_PATHS.contact(publicId));
  const record = await getContact(publicId);
  if (!record) notFound();
  const [notes, activity] = await Promise.all([
    listInternalNotes("contact", record.public_id),
    listOperationalActivity("contact", record.public_id),
  ]);
  return (
    <main>
      <PageHeader
      ...
      />
      <ContactForm record={record} />
      <SectionPanel title="Internal notes">
        <InternalNoteForm entityKind="contact" entityPublicId={record.public_id} />
        {notes.length === 0 ? (
