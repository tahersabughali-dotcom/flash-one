...
export type DeveloperWorkspaceFormState = {
  error: string | null;
};
...
async function requireAssignedProject(projectPublicId: string) {
  const { session } = await requireCompletedOnboarding(
    ACCOUNT_PATHS.developerProject(projectPublicId),
  );
  ...
  const { data: assigned } = await supabase.rpc("is_assigned_project_developer", {
...
export async function uploadDeveloperProjectFileAction(
  _previous: DeveloperWorkspaceFormState,
  formData: FormData,
): Promise<DeveloperWorkspaceFormState> {
  const projectPublicId = String(formData.get("projectPublicId") || "");
  const loaded = await requireAssignedProject(projectPublicId);
  ...
  redirect(`${ACCOUNT_PATHS.developerProject(projectPublicId)}#files`);
}

export async function sendDeveloperProjectMessageAction(
  _previous: DeveloperWorkspaceFormState,
  formData: FormData,
): Promise<DeveloperWorkspaceFormState> {
  ...
  const loaded = await requireAssignedProject(parsed.data.projectPublicId);
  ...
  const { error } = await loaded.supabase.from("conversation_messages").insert({
    ...
    sender_kind: "developer",
    body: parsed.data.body,
  });
  if (error) {
    return { error: mapError(error.message) };
  }
  redirect(`${ACCOUNT_PATHS.developerProject(parsed.data.projectPublicId)}#conversation`);
}
